...
namespace Aimp.Migration.DateTimeBugFix
{
    public class AimpService : BaseHttpServer<IAimpWcfService>
    {
        private void _SetHeaderRequestLoginAndPassword()
        {
            ...
            headers.Add("login", Properties.Settings.Default.login);

            headers.Add("password", Properties.Settings.Default.password);
        }
        public AimpService()
            : base(new Uri(Properties.Settings.Default.uri))
        {
